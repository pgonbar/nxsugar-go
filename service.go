...
// or result): keys listed in redactParams are replaced by a marker keeping the
// original size, and every string longer than paramsMaxLen (when > 0) is
// truncated to that length plus "...". The original structure is preserved so
// log lines stay valid JSON.
func sanitizeParams(v interface{}, redactParams []string, paramsMaxLen int) interface{} {
	if v == nil {
		return nil
	}
	...
	return sanitizeValue(v, redact, paramsMaxLen, 0)
}

func sanitizeValue(v interface{}, redact map[string]bool, maxLen, depth int) interface{} {
	// Guard against pathological nesting (e.g. self-referencing structures).
	if depth > 32 {
		return v
	}
	switch t := v.(type) {
	case map[string]interface{}:
		...
		for k, val := range t {
			if redact[k] {
				out[k] = fmt.Sprintf("[redacted len=%d]", valueLen(val))
				continue
			}
			out[k] = sanitizeValue(val, redact, maxLen, depth+1)
		}
		return out
	case []interface{}:
		out := make([]interface{}, len(t))
		for i, val := range t {
			out[i] = sanitizeValue(val, redact, maxLen, depth+1)
		}
		return out
	case string:
...
// valueLen returns the size of a value in bytes, used by the redaction marker.
...
const (
	defaultLogParamsMaxLen = 256
)

// sanitizeAndView applies the per-method logging policy to a value (task params
// or result): keys listed in redactParams are replaced by a marker keeping the
// original size, and every string longer than paramsMaxLen (when > 0) is
// truncated to that length plus "...". The original structure is preserved so
// log lines stay valid JSON.
func sanitizeAndView(v interface{}, redactParams []string, paramsMaxLen int) interface{} {
	if paramsMaxLen == 0 {
		paramsMaxLen = defaultLogParamsMaxLen
	}
	return sanitizeParams(v, redactParams, paramsMaxLen)
}
...
func ReplyToWrapper(f func(*Task) (interface{}, *JsonRpcErr)) func(*Task) (interface{}, *JsonRpcErr) {
	return func(t *Task) (interface{}, *JsonRpcErr) {
		...
		_, err := t.Accept()
		if err != nil {
			t.Service.LogWithFieldsCtx(t.Ctx, WarnLevel, ei.M{"type": "reply_to_accept"}, "could not accept task: %s", err.Error())
		} else if repTy == "pipe" {
			if pipe, err := t.GetConn().PipeOpen(repPath); err != nil {
				t.Service.LogWithFieldsCtx(t.Ctx, WarnLevel, ei.M{"type": "reply_to_accept"}, "could not open received pipeId (%s): %s", repPath, err.Error())
				...
				t.Service.LogWithFieldsCtx(t.Ctx, WarnLevel, ei.M{"type": "reply_to_accept"}, "error writing response to pipe: %s", err.Error())
			}
			...
			if _, err := t.GetConn().TaskPush(repPath, map[string]interface{}{"result": res, "error": errm, "task": map[string]interface{}{"path": t.Path, "method": t.Method, "params": t.Params, "tags": t.Tags}}, time.Second*30, &nexus.TaskOpts{Detach: true}); err != nil {
				t.Service.LogWithFieldsCtx(t.Ctx, WarnLevel, ei.M{"type": "reply_to_accept"}, "could not push response task to received path (%s): %s", repPath, err.Error())
			}
		}
		return res, errm
	}
...
func (s *Service) taskPull(n int) {
	for {
		...
		go func() {
			...
			// OTel: extract the W3C traceparent from params and start a server span.
			...
			if m.resSchema != nil && wtask.Tags["@local-response-result"] != nil {
				...
				if err != nil {
					s.LogWithFieldsCtx(wtask.Ctx, ErrorLevel, ei.M{"type": "output_schema_error"}, "jsonschema result validation failed: %s", err.Error())
				} else if !result.Valid() {
					s.LogWithFieldsCtx(wtask.Ctx, ErrorLevel, ei.M{"type": "output_schema_error"}, "jsonschema result validation failed: %s", schemaValidationErr(result))
				}
			}
			...
			if m.errSchema != nil && wtask.Tags["@local-response-error"] != nil {
				...
				if err != nil {
					s.LogWithFieldsCtx(wtask.Ctx, ErrorLevel, ei.M{"type": "output_schema_error"}, "jsonschema error validation failed: %s", err.Error())
				} else if !result.Valid() {
					s.LogWithFieldsCtx(wtask.Ctx, ErrorLevel, ei.M{"type": "output_schema_error"}, "jsonschema error validation failed: %s", schemaValidationErr(result))
				}
			}
...
// LogCtx is the context-aware variant of Log; when ctx carries an OTel span,
// trace_id / span_id are auto-injected by the slog handler.
func (s *Service) LogCtx(ctx context.Context, level string, message string, args ...interface{}) {
	fields := map[string]interface{}{}
	connid := s.getConnid()
	if connid != "" {
		fields["connid"] = connid
	}
	LogWithFieldsCtx(ctx, level, s.Name+"/"+s.Path, fields, message, args...)
}
...
func (s *Service) LogWithFieldsCtx(ctx context.Context, level string, fields map[string]interface{}, message string, args ...interface{}) {
	if fields == nil {
		fields = map[string]interface{}{}
	}
	connid := s.getConnid()
